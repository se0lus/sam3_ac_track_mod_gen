"""Stage 7a: Initialize manual walls directory from stage 7 output.

Copies stage 7 results into 07a_manual_walls/ for manual editing via the
wall editor.  Existing edits in 7a are preserved (never overwritten).
"""
...
def run(config: PipelineConfig) -> None:
    """Initialize 07a_manual_walls from stage 7 output."""
    logger.info("=== Stage 7a: Initialize manual walls ===")
    ...
    if _copy_if_missing(src_walls, dst_walls):
        logger.info("Copied walls.json from stage 7")
    elif os.path.isfile(dst_walls):
        logger.info("walls.json already exists in 7a, preserving edits")
    else:
        logger.warning("No walls.json found in stage 7 output")
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="Stage 7a: Initialize manual walls")
